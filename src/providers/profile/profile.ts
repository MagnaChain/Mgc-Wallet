...
export class ProfileProvider {
  ...
  public setWalletOrder(walletId: string, index?: number, coin?: string): void {
    if (index === null) {
      ...
      let maxIndex = _.maxBy(wallets, 'order')
        ? _.maxBy(wallets, 'order').order
        : null;
      index = maxIndex != null ? maxIndex + 1 : 0;
    }
    this.persistenceProvider.setWalletOrder(walletId, index);
    this.logger.debug(
      'Wallet new order stored ' + this.wallet[walletId].name + ': ' + index
    );
  ...
  private getWalletOrder(wallet): Promise<any> {
    return new Promise((resolve, reject) => {
      this.persistenceProvider
        .getWalletOrder(wallet.credentials.walletId)
        .then(order => {
